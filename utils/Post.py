...
class Post:
    ...
    def get_post(self,number):
        return self.get_posts(number,1)[0]
    def get_posts(self,place,number):
      data = requests.post("https://www.quora.com/graphql/gql_para_POST?q=UserProfilePostsList_Posts_Query", 
        headers= {
          "accept": "*/*",
        ...
        json = {"queryName":"UserProfilePostsList_Posts_Query","variables":{"uid":int(self.profile.uid),"first":number,"after":int(place)-2},"extensions":{"hash":"8553d86bef0811de7339f1c7b08a12bb886f980bb2090b84819c8ec40db398cb"}}
        
      )
      body = json.loads(data.text)
      posts = body["data"]["user"]["postsConnection"]["edges"]
      user_posts =[]
      for p in posts:
          user_posts.append(p["node"]["url"])
      if len(user_posts) == 0:
          return ["no postes"]
      return user_posts