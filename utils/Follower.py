import requests
import json
# get peoples that follow user
from utils.Profile import Profile
class Follower:
    def __init__(self,username):
        self.profile = Profile(username)
    def get_followers(self,number,place):  
      data = requests.post("https://www.quora.com/graphql/gql_para_POST?q=UserProfileFollowers_ProfileTopics_Query", 
        headers= {
          "accept": "*/*",
        ...
        json = {"queryName":"UserProfileFollowers_ProfileTopics_Query","variables":{"uid":self.profile.uid,"first":int(number),"after":int(place)-2},"extensions":{"hash":"29efc30d6197ac799ba6d790054b835cfd3f800f99d95c69e237e2420b7af4c8"}}
        
      )
      body = json.loads(data.text)
      follower = body["data"]["user"]["followerUsersConnection"]["edges"]
      follower_profile = []
      for f in follower:
          follower_profile.append("https://www.quora.com/profile/" + f["node"]["profileUrl"].split("/")[2])
