...
class Following:
    ...
    def get_following(self,number,place):
      data = requests.post("https://www.quora.com/graphql/gql_para_POST?q=UserProfileFollowingPeople_ProfileTopics_Query", 
        headers= {
          "accept": "*/*",
        ...
        json = {"queryName":"UserProfileFollowingPeople_ProfileTopics_Query","variables":{"uid":int(self.profile.uid),"first":int(number),"after":int(place)-2},"extensions":{"hash":"e77e82bd75228b6a5e85a91949096c30d25eb104d13a78b97a379eecee05d3cd"}}
        
      )
      body = json.loads(data.text)
      following = body["data"]["user"]["followingUsersConnection"]["edges"]
      following_profile = []
      for f in following:
          following_profile.append("https://www.quora.com/profile/" + f["node"]["profileUrl"].split("/")[2])
